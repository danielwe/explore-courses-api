...
class School:
    ...
    def get_department(self, idf: str) -> Department:
        ...
        def find_dept(dept, idf):
            return dept.name.lower() == idf or dept.code.lower() == idf

        idx = [idx for idx, dept in enumerate(self.departments) if find_dept(dept, idf)]
...
class Schedule:
    ...
    def __init__(self, elem: Element):
        ...
        self.instructors = tuple(
            Instructor(instr) for instr in elem.find("instructors")
        )
    ...
    def __repr__(self):
        ...
        return (
            f"{', '.join(self.days)}, {self.start_time} - {self.end_time} "
            f"at {self.location}"
        )
...
class Section:
    ...
    def __init__(self, elem: Element):
        ...
        self.schedules = tuple(Schedule(sched) for sched in elem.find("schedules"))

        self.attributes = tuple(Attribute(attr) for attr in elem.find("attributes"))
...
class Course:
    ...
    def __init__(self, elem: Element):
        ...
        self.repeatable = elem.findtext("repeatable") == "true"
        ...
        self.objectives = tuple(
            LearningObjective(obj) for obj in elem.find("learningObjectives")
        )

        self.final_exam = (
            True
            if elem.findtext(".//finalExamFlag") == "Y"
            else False
            if elem.findtext(".//finalExamFlag") == "N"
            else None
        )

        self.sections = tuple(Section(section) for section in elem.find("sections"))
        ...
        self.active = (
            True
            if elem.findtext(".//effectiveStatus") == "A"
            else False
            if elem.findtext(".//effectiveStatus") == "I"
            else None
        )
